...
import './ButtonPassportTakePhoto.css'
...
const ButtonPassportTakePhoto: React.FC<ButtonTakePhotoProps> = ({onSelected, type, onFileReceived, onFileRemoved }) => {
  ...
  useEffect(() => {
    const currentImage = type === 'passport' ? statementImage : passportImage;
    if (currentImage) {
  ...
  return (
      <div>
        {!imageData ? (
            <Link href={'qr/photo_pass'} className='take_photo_pass' >
